...
export function SignInClient() {
  ...
  const getDashboardPathByRole = (role?: string | null) => {
    switch (role) {
      ...
      case "pao":
        return withLocale("/dashboard/pao");
      default:
  ...
  const isRedirectAllowedForRole = (path: string, role?: string | null) => {
    ...
    if (role === "pao") {
      return pathWithoutLocale.startsWith("/dashboard/pao");
    }
    ...
    return (
      ...
      !pathWithoutLocale.startsWith("/dashboard/pao")
    );
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const loginResponse = await authService.login({
        ...
        expected_user_type: userType as "admin" | "agent" | "customer" | "pao",
      });
      ...
      if (
        redirectUrl &&
        isRedirectAllowedForRole(redirectUrl, resolvedUserType)
      ) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-plp-purple/5 via-white to-plp-pink/5 flex flex-col">
        ...
        <header className="border-b bg-white/50 backdrop-blur-sm sticky top-0 z-40">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            ...
            <Link
              href="/"
              className="flex items-center gap-1 text-gray-600 hover:text-plp-purple transition-colors"
            >
