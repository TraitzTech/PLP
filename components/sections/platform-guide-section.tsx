...
export function PlatformGuideSection({
  ...
  const withLocale = (link?: string) => {
    if (!link) return `${locale}/search`;
    if (link.startsWith("http")) return link;
    if (link.startsWith("/en") || link.startsWith("/fr")) return link;
    ...
    const load = async () => {
      ...
      if (showPopularSearches) {
        ...
        const normalized = Array.isArray(popular)
          ? popular
              .map((item: PopularSearchItem) => toSearch(item))
  ...
  const steps = useMemo(() => {
    ...
    return [
      {
        title: t("platformGuide.step1Title", "Search"),
        description: t(
          "platformGuide.step1Desc",
          "Use filters for city, price, and property type to quickly find the right listing.",
        ),
        accent: `${t("platformGuide.stepLabel", "Step")} 1`,
      },
      {
        title: t("platformGuide.step2Title", "Identify"),
        description: t(
          "platformGuide.step2Desc",
          "Open the listing details, verify photos and features, and compare options.",
        ),
        accent: `${t("platformGuide.stepLabel", "Step")} 2`,
      },
      {
        title: t("platformGuide.step3Title", "Contact Agent"),
        description: t(
          "platformGuide.step3Desc",
          "Reach out instantly through WhatsApp or direct contact details on the listing.",
        ),
        accent: `${t("platformGuide.stepLabel", "Step")} 3`,
      },
      {
        title: t("platformGuide.step4Title", "Book or Proceed"),
        description: t(
          "platformGuide.step4Desc",
          "Confirm your booking or continue the deal process safely with the provider.",
        ),
        accent: `${t("platformGuide.stepLabel", "Step")} 4`,
      },
    ];
  }, [settings.platform_how_steps_items, t]);
        ...
        <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-indigo-200">
              {t("platformGuide.eyebrow", "How It Works")}
            </p>
            <h2 className="mt-2 text-3xl font-bold leading-tight sm:text-4xl">
              {String(
                settings.platform_how_steps_title ||
                  t("platformGuide.title", "How the Platform Works"),
              )}
            </h2>
          ...
          <div className="flex items-center gap-3 rounded-full border border-white/10 bg-white/10 px-4 py-2">
            ...
            <span className="text-sm text-indigo-100">
              {t("platformGuide.trustBadge", "Trusted and transparent process")}
            </span>
          </div>
        </div>
          ...
          {steps.map((step, index) => {
            ...
            return (
              <div
                ...
                className="group relative h-full rounded-2xl border border-white/10 bg-white/5 p-5 shadow-lg backdrop-blur-sm"
              >
                <div className="mb-3 flex items-center justify-between">
                  <span className="text-xs font-semibold tracking-wide text-indigo-200">
                    {step.accent ||
                      `${t("platformGuide.stepLabel", "Step")} ${index + 1}`}
                  </span>
        ...
        {showPopularSearches && (
          <div className="mt-12 rounded-2xl border border-white/15 bg-white/5 p-6">
            <h3 className="text-2xl font-bold text-white">
              {String(
                settings.platform_popular_searches_title ||
                  t("platformGuide.popularTitle", "Popular Searches"),
              )}
            </h3>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-xs text-indigo-200">
                        {item.count || 0}{" "}
                        {t("platformGuide.searchesLabel", "searches")}
                      </span>
