...
import { homepageService, type HomepagePayload, type HomepageProperty } from '@/services/homepageService';
...
function PropertyMiniCard({ property, locale }: { property: HomepageProperty; locale: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2 p-4">
        <h3 className="line-clamp-1 text-base font-semibold text-gray-900">{property.title}</h3>
        <p className="flex items-center gap-1 text-sm text-gray-600">
          <MapPin className="h-4 w-4" />
          {property.city || 'City not specified'}
        </p>
        <p className="text-sm font-semibold text-blue-700">{formatCurrency(property.discount_price || property.price)}</p>
      </div>
    </Link>
  );
}
      ...
      <section className="bg-white py-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-blue-600">Why Use PLP</p>
            <h2 className="mt-3 text-3xl font-bold text-gray-900 sm:text-4xl">Why Use Property Listing Portal?</h2>
          </div>
          <div className="mx-auto mt-10 grid max-w-4xl grid-cols-1 gap-4 sm:grid-cols-2">
            {whyUsePoints.map((point, index) => (
              <div key={`${point}-${index}`} className="flex items-start gap-3 rounded-xl border border-blue-100 bg-blue-50/50 p-4">
        ...
        <div className="container mx-auto space-y-8 px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-8 lg:flex-row">
            <div className="w-full rounded-2xl border border-gray-200 bg-white p-6 lg:w-5/12">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">Featured Agent</p>
              <h3 className="mt-2 text-2xl font-bold text-gray-900">Agent of the Week</h3>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="relative h-16 w-16 overflow-hidden rounded-full bg-emerald-100">
                      {getProfilePhotoUrl(data.featured_agent.profile_photo) ? (
                        <Image
                          src={getProfilePhotoUrl(data.featured_agent.profile_photo) || ''}
                        ...
                        <div className="flex h-full w-full items-center justify-center text-xl font-bold text-emerald-700">
                          {data.featured_agent.name?.charAt(0)?.toUpperCase() || 'A'}
                        </div>
                      )}
                    </div>
                    <div>
                      <p className="text-lg font-semibold text-gray-900">{data.featured_agent.name}</p>
                      <p className="text-sm text-gray-600">{data.featured_agent.city || 'Cameroon'}</p>
                    </div>
                  </div>
                  <div className="mt-4 grid grid-cols-1 gap-3 rounded-lg bg-white p-3 sm:grid-cols-2">
                    <div>
                      <p className="text-xs text-gray-500">City</p>
                      <p className="text-sm font-semibold text-gray-900">{data.featured_agent.city || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500">Listings</p>
                      <p className="text-sm font-semibold text-gray-900">{data.featured_agent.listings_count}</p>
                    </div>
                  </div>
                </div>
              ) : (
                <p className="mt-5 text-sm text-gray-500">No featured agent available yet.</p>
              )}
            </div>
            ...
            <div className="w-full rounded-2xl border border-gray-200 bg-white p-6 lg:w-7/12">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-orange-700">Fresh Listings</p>
                  <h3 className="mt-2 text-2xl font-bold text-gray-900">Recently Added Properties</h3>
                </div>
              ...
              {loading ? (
                ...
                <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {(data?.recent_properties || []).slice(0, 4).map((property) => (
                    <PropertyMiniCard key={property.id} property={property} locale={locale} />
                  ))}
                </div>
              )}
            </div>
      ...
      <section className="bg-white py-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8 flex flex-col gap-4 text-center sm:text-left">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-violet-700">Current Coverage</p>
            <h3 className="text-3xl font-bold text-gray-900 sm:text-4xl">Cities Covered</h3>
            <p className="text-sm text-gray-600">Starting with key cities and expanding steadily as we grow.</p>
          </div>

          <div className="mb-10 flex flex-wrap gap-3">
            {citiesCovered.map((city) => (
              <span key={city} className="rounded-full border border-violet-200 bg-violet-50 px-4 py-2 text-sm font-semibold text-violet-700">
                {city}
              </span>
            ))}
          </div>
          ...
          <div className="rounded-3xl bg-gradient-to-r from-slate-900 to-slate-800 p-6 sm:p-8">
            <h4 className="text-2xl font-bold text-white">Property Listing Portal in Numbers</h4>
            <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <div className="rounded-xl border border-white/20 bg-white/10 p-4">
                <Building2 className="h-5 w-5 text-amber-300" />
                <p className="mt-2 text-2xl font-bold text-white">{formatCompact(numbers.properties_listed)}</p>
                <p className="text-sm text-slate-200">Properties Listed</p>
              </div>
              <div className="rounded-xl border border-white/20 bg-white/10 p-4">
                <Users className="h-5 w-5 text-emerald-300" />
                <p className="mt-2 text-2xl font-bold text-white">{formatCompact(numbers.verified_agents)}</p>
                <p className="text-sm text-slate-200">Happy Customers</p>
              </div>
              <div className="rounded-xl border border-white/20 bg-white/10 p-4">
                <MapPin className="h-5 w-5 text-blue-300" />
                <p className="mt-2 text-2xl font-bold text-white">{formatCompact(numbers.cities_covered)}</p>
                <p className="text-sm text-slate-200">Cities</p>
              </div>
            </div>
          </div>
        </div>
